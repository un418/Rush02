...
/*   Updated: 2026/02/15 16:59:41 by adaferna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_arg(int argc, char **argv, char *filename)
{
	char default_filename[]="numbers.dict";
		...
		if ((argc == 2 && !ft_is_number(argv[1])) || (argc == 3 && !ft_is_number(argv[2])) ||  argc > 3)
		{
			write(2, "Error\n", 6);
			return (1);
		}
		...
		{
			free(filename);
			filename = malloc(sizeof(char) * ft_strlen(default_filename) + 1);
